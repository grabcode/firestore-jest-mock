...
describe('Single records versus queries', () => {
  ...
  test('it can set & merge new attr on record', async () => {
    const record = await db
      .collection('characters')
      .doc('krusty')
      .get();

    const data = record.set({ action: 'smokes' }, { merge: true });
    expect(data).toHaveProperty('action', 'smokes');
    expect(data).toHaveProperty('occupation', 'clown');
  });

  test('it can set complete new attr on record', async () => {
    const record = await db
      .collection('characters')
      .doc('krusty')
      .get();

    const data = record.set({ action: 'smokes' });
    expect(data).toHaveProperty('action', 'smokes');
    expect(data).not.toHaveProperty('occupation', 'clown');
  });
